   ...
   public class Crawler
    {
        ...
        public void Crawl(string startUrl,string preurl)
        {
           
            Console.WriteLine("开始爬行了…");
            string starthtml = Download(startUrl);
            if (starthtml.Contains("<html"))
            {
                Parse(starthtml, preurl);    // 只爬取起始网站上的网页 只解析一次    
            }
            while (true)
            {               
                ...
                if (URLS==null || count > 10) break;
               

                ParallelLoopResult result = Parallel.ForEach<string>(URLS, str =>        //  并行处理
                {
                    Console.WriteLine("爬行" + str + "页面！");

                    string html = Download(str);
